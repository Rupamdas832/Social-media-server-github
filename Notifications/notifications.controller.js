const { NotificationData } = require("./notifications.model.js")
...
const getNotificationData = async (req, res) => {
  ...
  try {
    res.status(200).json({ success: true, notifications: notificationData.notifications });
  } catch (error) {
    console.log(error)
    res.status(500).json({ success: false, message: "Unable to retrive the NotificationData" })
  }
}
...
const addNotification = async (req, res) => {
  ...
  try {
    let notificationData = await NotificationData.findOne({ userId: userIdToUpdate })
    if (!notificationData) {
      return res.status(400).json({ success: false, message: "Notification data of the user couldn't be found. Try again" })
    }
...
module.exports = { getNotificationDataByUserId, getNotificationData, addNotification }